...
TaskManager.defineTask(STEPS_NOTIFICATION_TASK, async () => {
    try {
        ...
        if (steps >= goal) {
            await Notifications.scheduleNotificationAsync({
                content: { title: "🎉 هدف الخطوات مكتمل!", body: `رائع! لقد حققت هدفك اليومي وهو ${goal.toLocaleString()} خطوة.`, sound: true, },
                trigger: null,
            });
    ...
    } catch (error) { console.error("Error occurred in background steps task:", error); return BackgroundFetch.BackgroundFetchResult.Failed; }
});
...
// ... (Animation and helper functions remain same) ...
...
const DateNavigator = ({ selectedDate, onDateSelect, referenceToday, theme, t, language }) => {
    ...
    let todayDiff = todayDayIndex - startOfWeekIndex;
        ...
        <View style={styles.dateNavContainer(theme)}>
            ...
            <View style={[styles.dateNavHeader, { flexDirection: 'row' }]}>
                ...
                <TouchableOpacity onPress={handleNextWeek} style={styles.arrowButton} disabled={isNextDisabled}>
                    <Ionicons name={isArabic ? "chevron-back-outline" : "chevron-forward-outline"} size={24} color={isNextDisabled ? theme.disabled : theme.primary} />
                </TouchableOpacity>
            </View>
            ...
            <View style={[styles.datesContainer, { flexDirection: 'row' }]}>
                {displayDates.map((date, index) => {
                    ...
                    return (
                        <TouchableOpacity key={`date-${index}`} onPress={() => onDateSelect(date)} disabled={isFutureDate}>
...
const SummaryCard = ({ data, dailyGoal, theme, t }) => { 
    ...
    const animatedPathProps = useAnimatedProps(() => { const angle = animatedProgress.value * 360; if (angle < 0.1) { return { d: '' }; } return { d: describeArc(SIZE / 2, SIZE / 2, CENTER_RADIUS, 0, angle), }; }); 
    const indicatorAnimatedStyle = useAnimatedStyle(() => { const angleRad = (animatedProgress.value * 360 - 90) * (Math.PI / 180); const x = CENTER_RADIUS * Math.cos(angleRad); const y = CENTER_RADIUS * Math.sin(angleRad); return { transform: [{ translateX: x }, { translateY: y }], }; }); 
    return (
        <View style={[styles.card(theme), { alignItems: 'center' }]}>
            <View style={[styles.summaryCircleContainer, { width: SIZE, height: SIZE, direction: 'ltr' }]}>
                ...
                <Animated.View style={[styles.progressIndicatorDot(theme), { width: INDICATOR_SIZE, height: INDICATOR_SIZE, borderRadius: INDICATOR_SIZE / 2, position: 'absolute', left: (SIZE - INDICATOR_SIZE) / 2, top: (SIZE - INDICATOR_SIZE) / 2, }, indicatorAnimatedStyle]} />
...
const NutrientSummaryCard = ({ data, theme, t, language }) => { 
    ...
    return (<View style={styles.card(theme)}>{nutrients.map((nutrient, index) => (<NutrientRow key={index} {...nutrient} theme={theme} language={language} />))}</View>); 
};
...
const DailyFoodLog = ({ items, onPress, theme, t, language }) => { 
    const isEmpty = !items || items.length === 0; const MAX_PREVIEW_IMAGES = 4; 
            ...
            <View style={[styles.card(theme), styles.dailyLogCard]}>
                <View style={[styles.dailyLogContentContainer, { flexDirection: 'row' }]}>
                    ...
                    <View style={[styles.dailyLogLeftContainer, { flexDirection: 'row' }]}>
                        ...
                        ) : (<Ionicons name={isRTL ? "chevron-back-outline" : "chevron-forward-outline"} size={24} color={theme.textSecondary} />)}
                    </View>
                </View>
            </View>
        </TouchableOpacity>
...
const AddFoodModal = ({ visible, onClose, onFoodSelect, mealKey, theme, t, language }) => { 
    const [query, setQuery] = useState(''); const [results, setResults] = useState([]); const [loading, setLoading] = useState(false); const [fetchingDetailsId, setFetchingDetailsId] = useState(null); 
    const mealTranslations = { breakfast: t('breakfast'), lunch: t('lunch'), dinner: t('dinner'), snacks: t('snacks') }; 
    const mealTitle = mealTranslations[mealKey] || '...'; 
    const handleClose = () => { setQuery(''); setResults([]); setLoading(false); setFetchingDetailsId(null); onClose(); }; 
    const searchSpoonacular = async (searchQuery) => { try { const response = await fetch(`https://api.spoonacular.com/food/ingredients/search?query=${searchQuery}&number=15&apiKey=${SPOONACULAR_API_KEY}`); const data = await response.json(); return data.results ? data.results.map(item => ({ ...item, source: 'spoonacular' })) : []; } catch (error) { console.error("Spoonacular Search API Error:", error); return []; } }; 
...
function DiaryScreen({ navigation, route, setHasProgress, theme, t, language }) { 
    ...
    const loadAllData = useCallback(async () => { try { let goalToSet = 2000; if (passedGoal) { goalToSet = passedGoal; const profileJson = await AsyncStorage.getItem('userProfile'); const profileData = profileJson ? JSON.parse(profileJson) : {}; profileData.dailyGoal = passedGoal; await AsyncStorage.setItem('userProfile', JSON.stringify(profileData)); } else { const profileJson = await AsyncStorage.getItem('userProfile'); const savedProfile = profileJson ? JSON.parse(profileJson) : null; if (savedProfile && savedProfile.dailyGoal) { goalToSet = savedProfile.dailyGoal; } else { const { data: { user } } = await supabase.auth.getUser(); if (user?.user_metadata?.daily_goal) { goalToSet = user.user_metadata.daily_goal; const profileToSave = savedProfile || {}; profileToSave.dailyGoal = goalToSet; await AsyncStorage.setItem('userProfile', JSON.stringify(profileToSave)); } } } setDailyGoal(goalToSet); const settingsJson = await AsyncStorage.getItem('waterSettings'); setWaterGoal(settingsJson ? (JSON.parse(settingsJson).goal || 8) : 8); const dateKey = formatDateKey(selectedDate); const dayJson = await AsyncStorage.getItem(dateKey); let currentDayData = dayJson ? JSON.parse(dayJson) : { ...EMPTY_DAY_DATA }; const weightHistoryJson = await AsyncStorage.getItem('weightHistory'); const weightHistory = weightHistoryJson ? JSON.parse(weightHistoryJson) : []; if (weightHistory.length > 0) { weightHistory.sort((a, b) => new Date(a.date) - new Date(b.date)); const endOfDay = new Date(selectedDate); endOfDay.setHours(23, 59, 59, 999); const weightForDate = weightHistory.filter(entry => new Date(entry.date) <= endOfDay).pop(); currentDayData.displayWeight = weightForDate ? weightForDate.weight : 0; } else { currentDayData.displayWeight = 0; } setDailyData(currentDayData); } catch (e) { console.error("Failed to load data on focus:", e); setDailyData(EMPTY_DAY_DATA); setDailyGoal(2000); } }, [selectedDate, passedGoal]);
    ...
    return ( 
        <SafeAreaView style={styles.rootContainer(theme)}>
            ...
            <AddFoodModal visible={isFoodModalVisible} onClose={() => setFoodModalVisible(false)} onFoodSelect={handleFoodSelectedFromModal} mealKey={currentMealKey} theme={theme} t={t} language={language} />
...
const MagicLineTabBar = ({ state, descriptors, navigation, theme, t, language }) => {
    ...
    useFocusEffect(useCallback(() => {
        const loadProfileImage = async () => {
            ...
            } catch (e) { console.error("Failed to load profile image for tab bar:", e); }
        };
        loadProfileImage();
    }, []));
...
function DiaryStackNavigator({ setHasProgress, theme, t, language }) { 
  return ( 
...
const styles = StyleSheet.create({ 
    ...
    modalHeader: (theme) => ({ justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: '#eee', backgroundColor: theme.card }), 
    ...
    searchInput: (theme) => ({ flex: 1, height: 50, backgroundColor: theme.background, borderRadius: 10, paddingHorizontal: 15, fontSize: 16, color: theme.textPrimary }), 
    searchButton: (theme) => ({ width: 50, height: 50, backgroundColor: theme.primary, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }), 
    resultItem: { justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'transparent', padding: 20, borderBottomWidth: 1, borderBottomColor: '#eee' }, 
    ...
    smallCardContent: { }, 
    waterVisualizerContainer: { alignItems: 'center', justifyContent: 'flex-start', flexWrap: 'wrap', rowGap: 5, }, 
