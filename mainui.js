...
TaskManager.defineTask(STEPS_NOTIFICATION_TASK, async () => {
    // ... (نفس كود التاسك القديم)
    try {
        const settingsRaw = await AsyncStorage.getItem('reminderSettings');
        const settings = settingsRaw ? JSON.parse(settingsRaw) : {};
        if (!settings.stepsGoal || !settings.stepsGoal.enabled) return BackgroundFetch.BackgroundFetchResult.NoData;
        ...
        if (hasSentToday) return BackgroundFetch.BackgroundFetchResult.NoData;
        const savedGoal = await AsyncStorage.getItem('stepsGoal');
        const goal = savedGoal ? parseInt(savedGoal, 10) : 10000;
        const isAvailable = await Pedometer.isAvailableAsync();
        if (!isAvailable) return BackgroundFetch.BackgroundFetchResult.Failed;
        const { steps } = await Pedometer.getStepCountAsync(start, new Date());
        if (steps >= goal) {
            await Notifications.scheduleNotificationAsync({ content: { title: "🎉 هدف الخطوات مكتمل!", body: `رائع! لقد حققت هدفك اليومي وهو ${goal.toLocaleString()} خطوة.`, sound: true, }, trigger: null, });
...
// ✅ تم التعديل: إجبار الترتيب على الانعكاس لو اللغة عربي
const DateNavigator = ({ selectedDate, onDateSelect, referenceToday, theme, t, language }) => {
    ...
    return (
        <View style={styles.dateNavContainer(theme)}>
            <View style={[styles.dateNavHeader, rowStyle]}>
                <TouchableOpacity onPress={handlePrevWeek} style={styles.arrowButton}>
                    <Ionicons name={prevIcon} size={24} color={theme.primary} />
                </TouchableOpacity>
                ...
                <TouchableOpacity onPress={handleNextWeek} style={styles.arrowButton} disabled={isNextDisabled}>
                    <Ionicons name={nextIcon} size={24} color={isNextDisabled ? theme.disabled : theme.primary} />
                </TouchableOpacity>
            </View>
            <View style={[styles.weekContainer, rowStyle]}>
                {weekDays.map((day, index) => <Text key={index} style={styles.weekDayText(theme)}>{day}</Text>)}
            </View>
            <View style={[styles.datesContainer, rowStyle]}>
    ...
    return (
        <View style={[styles.card(theme), { alignItems: 'center' }]}>
            <View style={[styles.summaryCircleContainer, { width: SIZE, height: SIZE, direction: 'ltr' }]}>
                ...
                <View style={[styles.summaryTextContainer, {transform: [{scaleX: 1}]}]}>
...
// ✅ تم التعديل: NutrientRow بيعكس الاتجاه يدوياً
const NutrientRow = ({ label, consumed, goal, color, unit = 'جم', isLimit = false, theme, isRTL }) => { 
    ...
    return (
        <View style={styles.nutrientRowContainer}>
            ...
            {/* البار دايماً بيبدأ من الشمال، لو عربي ممكن نعكسه بـ ScaleX */}
...
const NutrientSummaryCard = ({ data, theme, t, language }) => { 
    ...
    return (<View style={styles.card(theme)}>{nutrients.map((nutrient, index) => (<NutrientRow key={index} {...nutrient} theme={theme} isRTL={isRTL} />))}</View>); 
};

// ✅ تم التعديل: FoodLogItem بيعكس الاتجاه يدوياً
const FoodLogItem = ({ item, theme, t, showMacros = true, isRTL }) => { 
    ...
    const marginStyle = isRTL ? { marginLeft: 15 } : { marginRight: 15 };

    return (
        <View style={[styles.foodLogItemContainer, rowStyle]}>
            {imageSource ? (<Image source={imageSource} style={[styles.foodLogItemImage, marginStyle]} />) : (<View style={[styles.foodLogItemImagePlaceholder(theme), marginStyle]}><Ionicons name="restaurant-outline" size={24} color={theme.primary} /></View>)}
            <View style={styles.foodLogItemDetails}>
                <View style={[styles.foodLogItemHeader, rowStyle]}>
                    <Text style={[styles.foodLogItemName(theme), textStyle]} numberOfLines={1}>{item.name}</Text>
                    <Text style={[styles.foodLogItemCalories(theme), isRTL ? {marginRight: 8} : {marginLeft: 8}]}>{Math.round(item.calories)} {t('kcal_unit')}</Text>
                </View>
                {showMacros && (<View style={[styles.foodLogItemMacros, rowStyle, {justifyContent: isRTL ? 'flex-end' : 'flex-start'}]}>
                    {/* هنا المغذيات ممكن نسيبها LTR عشان الأرقام بس ممكن نعكس الكونتينر */}
                     {/* للتبسيط هنسيبهم في سطر واحد ونعكس ترتيبهم لو عايزين */}
                    <Text style={[styles.macroText(theme), isRTL ? {marginLeft: 15} : {marginRight: 15}]}><Text style={{ color: theme.protein }}>{t('p_macro')}</Text>{Math.round(item.p || 0)}g</Text>
                    <Text style={[styles.macroText(theme), isRTL ? {marginLeft: 15} : {marginRight: 15}]}><Text style={{ color: theme.carbs }}>{t('c_macro')}</Text>{Math.round(item.c || 0)}g</Text>
                    <Text style={[styles.macroText(theme), isRTL ? {marginLeft: 15} : {marginRight: 15}]}><Text style={{ color: theme.fat }}>{t('f_macro')}</Text>{Math.round(item.f || 0)}g</Text>
                </View>)}
            </View>
        </View>
    ); 
                        ...
                        {!isEmpty ? (
                            <View style={[styles.foodPreviewContainer, rowStyle]}>
                                ...
                                {items.length > MAX_PREVIEW_IMAGES && (
                                    <View style={[styles.previewCounterCircle(theme), { zIndex: 0 }, isRTL ? {marginRight: -18} : {marginLeft: -18}]}>
                                        <Text style={styles.previewCounterText(theme)}>+{items.length - MAX_PREVIEW_IMAGES}</Text>
                                    </View>
                                )}
                            </View>
                        ) : (<Ionicons name={arrowIcon} size={24} color={theme.textSecondary} />)}
                    </View>
                </View>
            </View>
        </TouchableOpacity>
...
const MealLoggingSection = ({ title, iconName, items, onAddPress, mealKey, isEditable, theme, t, language }) => { 
    ...
    return (
        <View style={styles.card(theme)}>
            <View style={[styles.mealSectionHeader, rowStyle]}>
                <View style={[styles.mealSectionHeaderLeft, rowStyle]}>
                    <Ionicons name={iconName} size={24} color={theme.primary} style={marginStyle} />
            ...
            {items && items.length > 0 && items.map((item, index) => (<FoodLogItem key={`${item.id}-${index}`} item={item} showMacros={false} theme={theme} t={t} isRTL={isRTL} />))}
            
            {items && items.length > 0 && (<View style={[styles.mealMacrosContainer(theme), rowStyle, {justifyContent: 'flex-start'}]}>
                <View style={[styles.macroSummaryItem, isRTL ? {marginLeft: 20} : {marginRight: 20}]}><Text style={styles.macroSummaryText(theme)}>{t('fat')}: {Math.round(totalMacros.f)} {t('g_unit')}</Text></View>
                <View style={[styles.macroSummaryItem, isRTL ? {marginLeft: 20} : {marginRight: 20}]}><Text style={styles.macroSummaryText(theme)}>{t('carbs')}: {Math.round(totalMacros.c)} {t('g_unit')}</Text></View>
                <View style={[styles.macroSummaryItem, isRTL ? {marginLeft: 20} : {marginRight: 20}]}><Text style={styles.macroSummaryText(theme)}>{t('protein')}: {Math.round(totalMacros.p)} {t('g_unit')}</Text></View>
            </View>)}
            
            <TouchableOpacity style={[styles.smartAddButton(theme), !isEditable && styles.disabledButton(theme)]} onPress={() => onAddPress(mealKey)} disabled={!isEditable} >
                <Text style={styles.smartAddButtonText(theme)}>{t('add_to_meal', {meal: title})}</Text>
            </TouchableOpacity>
        </View>
    ); 
};

// ... AddFoodModal زي ما هو ...
...
// ✅ تم التعديل: الكروت الصغيرة
const SmallWeightCard = ({ weight, onPress, theme, t, isRTL }) => {
    const rowStyle = { flexDirection: isRTL ? 'row-reverse' : 'row' };
    const textStyle = { textAlign: isRTL ? 'right' : 'left' };
    return (<TouchableOpacity style={styles.smallCard(theme)} onPress={onPress}><View style={[styles.smallCardHeader, rowStyle]}><View style={[styles.smallCardIconContainer(theme)]}><Ionicons name="barbell-outline" size={20} color={theme.primary} /></View><Text style={[styles.smallCardTitle(theme), isRTL ? {marginRight: 8} : {marginLeft: 8}]}>{t('weight')}</Text></View><Text style={[styles.smallCardValue(theme), textStyle]}>{weight > 0 ? `${weight} ${t('kg_unit')}` : t('not_logged')}</Text></TouchableOpacity>);
};
const SmallWaterCard = ({ water, waterGoal, onPress, theme, t, isRTL }) => { 
    ...
    // بنعكس ترتيب النقط لو عربي
    const visualizerStyle = { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', flexWrap: 'wrap', rowGap: 5, direction: isRTL ? 'rtl' : 'ltr' };
    return (<TouchableOpacity style={styles.smallCard(theme)} onPress={onPress}><View style={[styles.smallCardHeader, rowStyle]}><View style={[styles.smallCardIconContainer(theme)]}><Ionicons name="water-outline" size={20} color={theme.primary} /></View><Text style={[styles.smallCardTitle(theme), isRTL ? {marginRight: 8} : {marginLeft: 8}]}>{t('water')}</Text></View><View style={visualizerStyle}>{drops.map(index => (<Ionicons key={index} name={index < filledDrops ? 'water' : 'water-outline'} size={22} color={index < filledDrops ? '#007BFF' : theme.disabled} style={styles.waterDropIcon} />))}</View></TouchableOpacity>); 
};
const SmallWorkoutCard = ({ totalCaloriesBurned = 0, onPress, theme, t, isRTL }) => { 
    const rowStyle = { flexDirection: isRTL ? 'row-reverse' : 'row' };
    const textStyle = { textAlign: isRTL ? 'right' : 'left' };
    return ( <TouchableOpacity style={styles.smallCard(theme)} onPress={onPress}><View style={[styles.smallCardHeader, rowStyle]}><View style={[styles.smallCardIconContainer(theme)]}><MaterialCommunityIcons name="run-fast" size={20} color={theme.primary} /></View><Text style={[styles.smallCardTitle(theme), isRTL ? {marginRight: 8} : {marginLeft: 8}]}>{t('workouts')}</Text></View><View style={[styles.smallCardContent, {alignItems: isRTL ? 'flex-end' : 'flex-start'}]}><Text style={[styles.smallCardValue(theme), textStyle]}>{totalCaloriesBurned > 0 ? `🔥 ${Math.round(totalCaloriesBurned)}` : t('not_logged')}</Text>{totalCaloriesBurned > 0 ? <Text style={[styles.smallCardSubValue(theme), textStyle]}>{t('burned_cal')}</Text> : null }</View></TouchableOpacity> ); 
};
...
function DiaryScreen({ navigation, route, setHasProgress, theme, t, language }) { 
    ...
    const alignItems = { alignItems: isRTL ? 'flex-end' : 'flex-start' };
...
const MagicLineTabBar = ({ state, descriptors, navigation, theme, t, language }) => {
    ...
    // معادلة حساب مكان المؤشر
    const getTabPosition = (index) => {
        // لو عربي، بنعكس المعادلة عشان الـ Index 0 (اليوميات) يبقى على اليمين
        if (isRTL) {
...
const styles = StyleSheet.create({ 
    ...
    dateNavHeader: {
        // تم التعديل ليقبل row-reverse من الستايل الممرر
        flexDirection: 'row',
    ...
    macroSummaryItem: { marginBottom: 5 }, 
    ...
    smallCardTitle: (theme) => ({ fontSize: 16, fontWeight: '600', color: theme.textPrimary, }), 
    smallCardValue: (theme) => ({ fontSize: 28, fontWeight: 'bold', color: theme.textPrimary, }), 
    smallCardSubValue: (theme) => ({ fontSize: 14, color: theme.textSecondary, marginTop: -5, }), 
    smallCardContent: { }, 
    ...
    foodLogItemImage: { width: 50, height: 50, borderRadius: 10, }, 
    foodLogItemImagePlaceholder: (theme) => ({ width: 50, height: 50, borderRadius: 10, backgroundColor: theme.background, justifyContent: 'center', alignItems: 'center', }), 
    ...
    foodLogItemCalories: (theme) => ({ fontSize: 14, fontWeight: '500', color: theme.primary, }), 
    foodLogItemMacros: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', marginTop: 6, }, 
    macroText: (theme) => ({ fontSize: 13, color: theme.textSecondary, marginBottom: 4, }), 
