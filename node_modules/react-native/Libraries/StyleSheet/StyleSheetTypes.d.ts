...
const MagicLineTabBar = ({ state, descriptors, navigation, theme, t, language }) => {
    ...
    // ✅ تحديد الاتجاه بناءً على اللغة فقط
    const isRTL = language === 'ar';

    // ✅ دالة حساب مكان المؤشر الأخضر
    const getTabPosition = (index) => {
        if (isRTL) {
            // في العربي (row-reverse): العنصر الأول (index 0) بيكون في أقصى اليمين
            // فالمؤشر لازم يروح لأقصى اليمين.
            // React Native Absolute Positioning بيبدأ من اليسار دائماً في ltr container
            return (TAB_COUNT - 1 - index) * TAB_WIDTH;
        } else {
            // في الإنجليزي (row): العنصر الأول في أقصى اليسار
    ...
    return (
        // ✅ استخدام direction: ltr دائماً لتجنب مشاكل النظام، مع قلب الـ flexDirection يدوياً
